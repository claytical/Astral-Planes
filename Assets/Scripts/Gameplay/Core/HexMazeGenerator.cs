...
public class HexMazeGenerator : MonoBehaviour
{
    ...
    public void GenerateMaze(Vector2Int center, MusicalPhase phase, float hollowRadius = 2.5f, bool toxic = false)
    {
        isToxic = toxic;
        fillMap.Clear();
...
// Spawn hex shields
    ...
    private IEnumerator StaggeredGrowth(List<(Vector2Int, Vector3)> cells)
    {
        foreach (var (grid, pos) in cells)
        {
            ...
            hexagons.Add(hex);
            HexagonShield hexScript = hex.GetComponent<HexagonShield>();
            if (hexScript != null)
            {
                if (isToxic)
                {
                    hexScript.SwitchType(HexagonShield.HexagonShieldType.Depleting);
                }
                else
                {
                    hexScript.SwitchType(HexagonShield.HexagonShieldType.Friendly);
                }
    ...
    private IEnumerator RegrowHexagon(Vector2Int gridPos, MusicalPhase phase)
    {
        ...
        if (hex.TryGetComponent<HexagonShield>(out var shield))
        {
            Debug.Log(($"hex at grid pos: {gridPos}"));
    ...
    public void TriggerRegrowth(Vector2Int gridPos, MusicalPhase phase)
    {
        ...
        Debug.Log($"⏳ Regrowing hex at {gridPos} for phase {phase}");

...
public class HarmonyDirector : MonoBehaviour
{
    ...
    [Header("Progression")]
    [SerializeField] private ChordProgressionProfile _profile;
    [SerializeField] private int _cursor = 0;
    [Header("Option C Behavior")]
    private bool _armedChordAdvance = false;
    private int  _pendingCharges = 0;
    public const int MaxCharges = 4;
    [Header("Rings & Timing")]
    [SerializeField] private int remixRings = 0;       // current rings the player holds (0..4)
    public const int MaxRings = 4;
    [SerializeField, Range(0f, 4f)] private float commitWindowBeats = 1f; // last N beats before downbeat
    [SerializeField] private bool burnIfOutsideWindow = false;            // consume rings even if mistimed (no chord change)
    ...
    [SerializeField] private bool seedFromProfileOnPhaseStart = false;    // leave false for Option C

    // per-track chord position (lets some tracks be on chord k while others on k-1)
    private readonly Dictionary<InstrumentTrack,int> _trackChordCursor = new();

    // preview/commit book-keeping
    private bool _previewActiveThisLoop = false;
    private int  _lastPreviewLoopIdx    = -1;
    private bool _heldThroughBoundary   = false;
    private bool _previewStartedInsideWindow = false;
    private int  _ringsArmedThisLoop    = 0;
    [SerializeField] private int _systemAdvanceStepsPending = 0;

    // Per-track progressive sequences (indices into _profile.chordSequence)
    ...
    // How many chords from the palette are globally "unlocked" so far.
    // Start at 1 (I is always present). Next preview will be IV (index 1), then V (index 2).
    private int _globalBuiltCount = 1;
    private int _previewChordIdx = -1;   // the palette index we're previewing this loop

    public enum ConflictPolicy { DeferPlayerToNextLoop, LetPlayerOvertake }
    ...
    private bool _hasPendingProfileSwap = false;
    ...
    {
        _systemAdvanceStepsPending += Mathf.Max(1, steps);
    }
    ...
    private bool AnyTrackHasNotes() =>
        tracks != null && tracks.tracks != null &&
        System.Array.Exists(tracks.tracks, t => t != null && t.GetPersistentLoopNotes().Count > 0);
    // ------------------------------------------------------------------
    // Init / Phase profile
    ...
    public void SetActiveProfile(ChordProgressionProfile profile, bool applyImmediately)
    {
        ...
        _globalBuiltCount = Mathf.Clamp(_profile.chordSequence.Count > 0 ? 1 : 0, 0, _profile.chordSequence.Count);
        _previewChordIdx = -1;
...
// HarmonyDirector.SetActiveProfile(...)
        ...
        {
            if (_profile.chordSequence.Count > 0)
            {
                var chord0 = _profile.chordSequence[0]; // I
    ...
    // ------------------------------------------------------------------
    // Preview (boost) → choose the *next unbuilt* chord in the palette
    public void BeginBoostArp(float secondsRemaining)
    {
        if (_profile == null || _profile.chordSequence == null || _profile.chordSequence.Count == 0 || arpeggiator == null || drums == null)
            return;
        ...
        _previewChordIdx = Mathf.Clamp(_globalBuiltCount, 0, _profile.chordSequence.Count - 1);
        ...
        arpeggiator.Begin(_profile.chordSequence[_previewChordIdx], Mathf.Max(0.05f, secondsRemaining));
    }

    public void CancelBoostArp() => arpeggiator?.Cancel();

    // ------------------------------------------------------------------
    // Loop boundary: resolve system/player changes, then step every track
    private void OnLoopBoundary()
    {
        if (_profile == null || tracks?.tracks == null) { ResetLoopFlags(); return; }

        int n = _profile.chordSequence.Count;
        var palette = _profile.chordSequence;
    ...
    // Call this from input: e.g., Vehicle or LocalPlayer → harmony.UseRemixRings(1)
    ...
    {
        if (_profile == null || tracks == null || tracks.tracks == null) return;
        var seq = _profile.chordSequence;
    ...
    private void ApplyToAllTracksWithOffset(int offset)
    {
        ...
        rotated.beatsPerChord  = _profile.beatsPerChord;
        rotated.chordSequence  = new List<Chord>(_profile.chordSequence.Count);
        int n = _profile.chordSequence.Count;
        for (int i = 0; i < n; i++)
            rotated.chordSequence.Add(_profile.chordSequence[(i + offset) % n]);
